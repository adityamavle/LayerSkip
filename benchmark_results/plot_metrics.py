...
# Load the CSV data
df = pd.read_csv('combined_metrics_full.csv')
...
plt.title('Accuracy vs. FLOPs per Token by Strategy')
plt.xlabel('FLOPs per Token (GFLOPs)')
plt.ylabel('Accuracy')
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend()
plt.tight_layout()
plt.savefig('plots/accuracy_vs_flops.png', dpi=300)
...
for i, strategy in enumerate(strategies):
    strategy_df = df[df['strategy'] == strategy].sort_values('value')
...
plt.title('Efficiency (Accuracy per FLOPs) by Strategy')
plt.xlabel('Strategy and Parameter')
plt.ylabel('Efficiency (Accuracy/FLOPs) × 10¹²')
plt.xticks(rotation=45, ha='right')
plt.grid(True, linestyle='--', alpha=0.7, axis='y')
plt.tight_layout()
plt.savefig('plots/efficiency_by_strategy.png', dpi=300)
...
ax1.bar(index - bar_width/2, df['accuracy'], bar_width, label='Accuracy', color='royalblue')
...
ax2 = ax1.twinx()
ax2.bar(index + bar_width/2, df['total_flops_readable'], bar_width, label='Total FLOPs', color='salmon')
...
plt.title('Accuracy and Computational Cost by Strategy')
...
plt.savefig('plots/accuracy_and_flops.png', dpi=300)

# 4. Latency comparison
plt.figure(figsize=(12, 8))
plt.bar(labels, df['time_per_token'], alpha=0.7, color='teal')
plt.title('Time per Token by Strategy')
plt.xlabel('Strategy')
